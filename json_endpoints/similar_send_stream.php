<?php
/**
 * SSE stream: send queued IRC requests from the similar-authors queue.
 *
...
$sentFile  = __DIR__ . '/../data/similar-author-books-sent.txt';
...
function getNewestMtime(): int {
    $body = curlGet(API_BASE . '/downloaded-files?dir=similar');
    if (!$body) return 0;
...
if (!file_exists($inputFile)) {
    sse('error', ['message' => 'Queue file not found: similar-author-books-results.txt']);
    exit;
}
...
foreach ($toSend as $idx => $cmd) {
    ...
    $body = curlGet(API_BASE . '/request-file?cmd=' . rawurlencode($cmd) . '&dest=similar');
