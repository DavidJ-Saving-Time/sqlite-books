...
<a href="listbooks.php?sort=<?= urlencode($sort) ?>&series_id=<?= urlencode($book['series_id']) ?>">
                        ...
                        <?php if (!empty($book['author_ids']) && !empty($book['authors'])): ?>
                            <?php
                                ...
                                foreach (array_slice(array_map(null, $ids, $names), 0, 3) as [$aid, $aname]) {
                                    $url = 'listbooks.php?sort=' . urlencode($sort) . '&author_id=' . urlencode($aid);
...
if ($isAjax) {
    ...
    if ($page < $totalPages) {
        echo '<div class="load-more" hx-get="listbooks.php?page=' . ($page + 1) . '&ajax=1" hx-trigger="revealed"></div>';
    }
    exit;
}
...
function buildBaseUrl(array $params, array $exclude = []): string {
    ...
    return 'listbooks.php?' . http_build_query($query);
}
...
<div class="container-fluid">      
        ...
        <div class="alert alert-info mb-3">
            ...
            <?php if ($recommendedOnly): ?>
                ...
                recommended only
            <?php endif; ?>
            <a class="btn btn-sm btn-secondary ms-2" href="listbooks.php?sort=<?= urlencode($sort) ?>">Clear</a>
        </div>
    <?php endif; ?>
...
<div class="col-md-12">
  <div id="contentArea">
      ...
      <?php if ($page < $totalPages): ?>
        <div class="load-more" hx-get="listbooks.php?page=<?= $page + 1 ?>&ajax=1" hx-trigger="revealed"></div>
      <?php endif; ?>
  </div>
</div>
        </div>
    ...
    <div id="alphabetBar" class="position-fixed bottom-0 start-0 end-0 bg-light text-center py-2">
        ...
        foreach (range('A', 'Z') as $letter) {
            ...
            $url = 'listbooks.php?' . http_build_query($letterParams);
            $active = ($authorInitial === $letter) ? 'fw-bold' : '';
            echo '<a href="' . htmlspecialchars($url) . '" class="mx-1 ' . $active . '">' . $letter . '</a>';
        }
        if ($authorInitial !== '') {
            $url = 'listbooks.php?' . http_build_query($baseLetterParams);
            echo '<a href="' . htmlspecialchars($url) . '" class="mx-1">Clear</a>';
        }
        ?>
    </div>
    
    <a href="#" id="backToTop" x-data="{show:false}" x-cloak x-show="show"
       x-on:scroll.window="show = window.scrollY > 200"
       x-on:click.prevent="sessionStorage.removeItem('lastItem'); window.location.href = window.location.pathname"
       class="btn btn-primary position-fixed end-0 m-3">
