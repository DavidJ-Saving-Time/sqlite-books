<?php
/**
 * Parses Locus-Award-for-Best-Science-Fiction-Novel.txt (Wikipedia HTML table) and
 * imports winners into book_awards. Every entry in this file is a winner.
 *
 * CLI:  php scripts/import_locus_sf_awards.php [/path/to/metadata.db]
 * Web:  load in browser while logged in (uses current user's library DB)
 *
 * Columns in source table: Year | Work | Author | Ref
 * "Not awarded" years are skipped automatically.
 *
 * Award created: "Locus Award for Best Science Fiction Novel"
 *
...
// ── Upsert award name ─────────────────────────────────────────────────────────
$pdo->prepare("INSERT OR IGNORE INTO awards (name) VALUES (?)")->execute(['Locus Best Science Fiction Novel']);
$locusId = (int)$pdo->query("SELECT id FROM awards WHERE name = 'Locus Best Science Fiction Novel'")->fetchColumn();

// ── Parse the HTML file ───────────────────────────────────────────────────────
$htmlFile = __DIR__ . '/../Locus-Award-for-Best-Science-Fiction-Novel.txt';
if (!file_exists($htmlFile)) {
    die("ERROR: Locus-Award-for-Best-Science-Fiction-Novel.txt not found at $htmlFile\n");
}
...
foreach ($rows as $row) {
    ...
    // Skip "Not awarded" rows
    $firstTd = $tds[0];
    if ($firstTd->hasAttribute('colspan') || stripos(trim($firstTd->textContent), 'not awarded') !== false) {
        echo "  [~] {$year}: not awarded — skipped\n";
        continue;
    }

    // Column order: Work (tds[0]), Author (tds[1])
    $novelTd = $tds[0];
    $iNodes  = $xpath->query('.//i', $novelTd);
    if ($iNodes->length > 0) {
        $title = trim(preg_replace('/\xc2\xa0/', ' ', $iNodes->item(0)->textContent));
    } else {
        $title = trim($novelTd->textContent);
    }
...
foreach ($entries as $e) {
    ...
    if ($changed > 0) {
        $inserted++;
        echo "  [+] {$e['year']} Locus Best Science Fiction Novel (won): {$book['title']}\n";
    } else {
        $skipped++;
        echo "  [=] {$e['year']} Locus Best Science Fiction Novel (won): {$book['title']} (already exists)\n";
    }
}
