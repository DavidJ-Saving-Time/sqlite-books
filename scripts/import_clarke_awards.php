<?php
/**
 * Parses Arthur-C-Clarke-award.txt (Wikipedia HTML table) and imports entries
 * into book_awards.
 *
 * CLI:  php scripts/import_clarke_awards.php [/path/to/metadata.db]
 ...
 *
 * Award created: "Arthur C. Clarke Award"
 *
...
// ── Upsert award name ─────────────────────────────────────────────────────────
$pdo->prepare("INSERT OR IGNORE INTO awards (name) VALUES (?)")->execute(['Arthur C. Clarke Award']);
$clarkeId = (int)$pdo->query("SELECT id FROM awards WHERE name = 'Arthur C. Clarke Award'")->fetchColumn();

// ── Parse the HTML file ───────────────────────────────────────────────────────
$htmlFile = __DIR__ . '/../Arthur-C-Clarke-award.txt';
if (!file_exists($htmlFile)) {
    die("ERROR: Arthur-C-Clarke-award.txt not found at $htmlFile\n");
}
...
foreach ($entries as $e) {
    ...
    $insert->execute([$book['id'], $clarkeId, $e['year'], $e['result']]);
    $changed = (int)$pdo->query("SELECT changes()")->fetchColumn();

    if ($changed > 0) {
        $inserted++;
        echo "  [+] {$e['year']} Arthur C. Clarke Award ({$e['result']}): {$book['title']}\n";
    } else {
        $skipped++;
        echo "  [=] {$e['year']} Arthur C. Clarke Award ({$e['result']}): {$book['title']} (already exists)\n";
    }
}
