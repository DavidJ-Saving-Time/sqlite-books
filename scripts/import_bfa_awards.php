<?php
/**
 * Parses BFA-Award-for-Best-Novel.txt (Wikipedia HTML table) and imports entries
 * into book_awards.
 *
 * CLI:  php scripts/import_bfa_awards.php [/path/to/metadata.db]
 ...
 *
 * Award created: "BFA Award for Best Novel"
 *
...
// ── Upsert award name ─────────────────────────────────────────────────────────
$pdo->prepare("INSERT OR IGNORE INTO awards (name) VALUES (?)")->execute(['BFA Award for Best Novel']);
$bfaId = (int)$pdo->query("SELECT id FROM awards WHERE name = 'BFA Award for Best Novel'")->fetchColumn();

// ── Parse the HTML file ───────────────────────────────────────────────────────
$htmlFile = __DIR__ . '/../BFA-Award-for-Best-Novel.txt';
if (!file_exists($htmlFile)) {
    die("ERROR: BFA-Award-for-Best-Novel.txt not found at $htmlFile\n");
}
...
foreach ($entries as $e) {
    ...
    $insert->execute([$book['id'], $bfaId, $e['year'], $e['result']]);
    $changed = (int)$pdo->query("SELECT changes()")->fetchColumn();

    if ($changed > 0) {
        $inserted++;
        echo "  [+] {$e['year']} BFA Award ({$e['result']}): {$book['title']}\n";
    } else {
        $skipped++;
        echo "  [=] {$e['year']} BFA Award ({$e['result']}): {$book['title']} (already exists)\n";
    }
}
