<?php
/**
 * Parses nebular-awards.txt (Wikipedia HTML table) and imports entries into book_awards.
 *
 * CLI:  php scripts/import_nebula_awards.php [/path/to/metadata.db]
 ...
 *
 * Award created: "Nebula Award" (Best Novel)
 *
 * Winners are identified by * after the author name.
 * Ties (multiple * winners in one year) are handled naturally.
...
// ── Upsert award name ─────────────────────────────────────────────────────────
$pdo->prepare("INSERT OR IGNORE INTO awards (name) VALUES (?)")->execute(['Nebula Award']);
$nebulaId = (int)$pdo->query("SELECT id FROM awards WHERE name = 'Nebula Award'")->fetchColumn();

// ── Parse the HTML file ───────────────────────────────────────────────────────
$htmlFile = __DIR__ . '/../nebular-awards.txt';
if (!file_exists($htmlFile)) {
    die("ERROR: nebular-awards.txt not found at $htmlFile\n");
}
...
foreach ($entries as $e) {
    ...
    $insert->execute([$book['id'], $nebulaId, $e['year'], $e['result']]);
    $changed = (int)$pdo->query("SELECT changes()")->fetchColumn();

    if ($changed > 0) {
        $inserted++;
        echo "  [+] {$e['year']} Nebula Award ({$e['result']}): {$book['title']}\n";
    } else {
        $skipped++;
        echo "  [=] {$e['year']} Nebula Award ({$e['result']}): {$book['title']} (already exists)\n";
    }
}
