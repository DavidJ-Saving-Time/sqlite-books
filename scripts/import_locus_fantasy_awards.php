<?php
/**
 * Parses Locus-Award-for-Best-Fantasy-Novel.txt (Wikipedia HTML table) and imports
 * winners into book_awards. Every entry in this file is a winner.
 *
 * CLI:  php scripts/import_locus_fantasy_awards.php [/path/to/metadata.db]
 * Web:  load in browser while logged in (uses current user's library DB)
 *
 * Columns in source table: Year | Novel | Author | Ref
 * "Not awarded" years are skipped automatically.
 *
 * Award created: "Locus Award for Best Fantasy Novel"
 *
...
// ── Upsert award name ─────────────────────────────────────────────────────────
$pdo->prepare("INSERT OR IGNORE INTO awards (name) VALUES (?)")->execute(['Locus Best Fantasy Novel']);
$locusId = (int)$pdo->query("SELECT id FROM awards WHERE name = 'Locus Best Fantasy Novel'")->fetchColumn();

// ── Parse the HTML file ───────────────────────────────────────────────────────
$htmlFile = __DIR__ . '/../Locus-Award-for-Best-Fantasy-Novel.txt';
if (!file_exists($htmlFile)) {
    die("ERROR: Locus-Award-for-Best-Fantasy-Novel.txt not found at $htmlFile\n");
}
...
foreach ($rows as $row) {
    ...
    // Skip "Not awarded" rows (colspan="2" on the first td, or text contains "not awarded")
    $firstTd = $tds[0];
...
foreach ($entries as $e) {
    ...
    if ($changed > 0) {
        $inserted++;
        echo "  [+] {$e['year']} Locus Best Fantasy Novel (won): {$book['title']}\n";
    } else {
        $skipped++;
        echo "  [=] {$e['year']} Locus Best Fantasy Novel (won): {$book['title']} (already exists)\n";
    }
}
