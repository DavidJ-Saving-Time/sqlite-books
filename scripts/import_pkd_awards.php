<?php
/**
 * Parses Philip-K-Dick-Award.txt (Wikipedia HTML table) and imports entries
 * into book_awards.
 *
 * CLI:  php scripts/import_pkd_awards.php [/path/to/metadata.db]
 ...
 *
 * Award created: "Philip K. Dick Award"
 *
 * Result detection (no * marker — uses formatting instead):
 *   won              — author <td> contains a <b> tag (bold author + title)
 *   special citation — row has background:lightyellow but author is NOT bold
 *   nominated        — no yellow background, no bold
 *
...
// ── Upsert award name ─────────────────────────────────────────────────────────
$pdo->prepare("INSERT OR IGNORE INTO awards (name) VALUES (?)")->execute(['Philip K. Dick Award']);
$pkdId = (int)$pdo->query("SELECT id FROM awards WHERE name = 'Philip K. Dick Award'")->fetchColumn();

// ── Parse the HTML file ───────────────────────────────────────────────────────
$htmlFile = __DIR__ . '/../Philip-K-Dick-Award.txt';
if (!file_exists($htmlFile)) {
    die("ERROR: Philip-K-Dick-Award.txt not found at $htmlFile\n");
}
...
foreach ($rows as $row) {
    ...
    // ── Detect result via formatting ──────────────────────────────────────────
    // Winner: author <td> has a <b> descendant (bold author)
    // Special citation: row is lightyellow but no bold
    // Nominated: no yellow, no bold
    $authorTd  = $tds[0];
    $hasBold   = $xpath->query('.//b', $authorTd)->length > 0;
    $rowStyle  = $row->getAttribute('style') ?? '';
    $isYellow  = str_contains($rowStyle, 'lightyellow');
...
foreach ($entries as $e) {
    ...
    $insert->execute([$book['id'], $pkdId, $e['year'], $e['result']]);
    $changed = (int)$pdo->query("SELECT changes()")->fetchColumn();

    if ($changed > 0) {
        $inserted++;
        echo "  [+] {$e['year']} Philip K. Dick Award ({$e['result']}): {$book['title']}\n";
    } else {
        $skipped++;
        echo "  [=] {$e['year']} Philip K. Dick Award ({$e['result']}): {$book['title']} (already exists)\n";
    }
}
